# 主模型
**基于中文数据集进行了一定调整和扩展，无法直接运行于Spider数据集**
* 代码参考了[Smbop](https://github.com/OhadRubin/SmBop)及[RAT-SQL](https://github.com/microsoft/rat-sql)的原文实现
* 配置文件目录: configs
* 配置文件名同数据集名: `dusql`、`nl2sql`、`cspider`
* tableqa数据集需要先运行`construct_query_tableqa.py`，完成后运行`nl2sql_postprocess.py`
* 运行训练: `exec.py`
* 运行推理: `eval.py`
* 从检查点中恢复训练：`resume.py`
