...
from smbop.utils.generate_query_toks import fix_date
import logging
...
class SmbopParser(Model):
    def __init__(
        ...
        self._action_dim = beam_encoder.get_output_dim() # TODO: 可是为什么是output_dim呢
        self._beam_size = beam_size
        self._n_schema_leafs = 15
        self._num_values = 15 # span数量
        self.err_list = []
        self._dataset_name = dataset_name
        self._use_relation = use_relation
        self._use_schema_structure = use_schema_structure
        ...
        self._emb_to_action_dim = torch.nn.Linear(
            in_features=self.q_emb_dim, # 混合序列编码的向量的维度
            out_features=self._action_dim, # 为什么是output_dim?
        )
        ...
        self._span_acc = Average()

        self._final_span_acc = Average()

        self._leafs_acc = Average()
        self._batch_size = -1
        self._device = None
        self._evaluate_func = partial(
            evaluate_single_using_json,
            db_dir=os.path.join(self._dataset_name, "db_content.json"), # FIXME: 硬编码
            table_file=os.path.join(self._dataset_name, "db_schema.json"), # FIXME: 硬编码
            value_match=self.value_pred
        )
    ...
    def set_flags(self):
        ...
        self.disentangle_cntx = self._misc_params.get("disentangle_cntx", True) # 这是什么意思？
    ...
    def _create_type_tensor(self):
        ...
        if 'nl2sql' in self._dataset_name:
            # banned_rules_str = '[["And", ["And", "Or"]], ["And", ["Or", "And"]], ["Or", ["Or", "And"]], ["Or", ["And", "Or"]]]'
            # banned_rules = json.loads(banned_rules_str)
            ...
            #     rule_tensor[i][j][k] = 0
            # 屏蔽掉And 与 Or 的组合
            and_idx = self._type_dict['And']
            or_idx = self._type_dict['Or']
            temp_keep_idx = self._type_dict['keep']
            rule_tensor[and_idx][and_idx][or_idx] = 0
            rule_tensor[and_idx][or_idx][and_idx] = 0
            rule_tensor[or_idx][and_idx][or_idx] = 0
            rule_tensor[or_idx][or_idx][and_idx] = 0
            for i_ in range(len(self._type_dict)):
                if i_ != temp_keep_idx:
                    rule_tensor[and_idx][or_idx][i_] = 0
                    rule_tensor[and_idx][i_][or_idx] = 0
                    rule_tensor[or_idx][and_idx][i_] = 0
                    rule_tensor[or_idx][i_][and_idx] = 0
            
            # 包含 R 类型在内的运算，参与运算的只能是Table. 选择和投影运算禁止嵌套
            table_idx = self._type_dict['Table']
            selection_idx = self._type_dict['Selection']
            project_idx = self._type_dict['Project']
            for i_ in range(len(self._type_dict)):
                if i_ != temp_keep_idx:
                    rule_tensor[selection_idx][selection_idx][i_] = 0
                    rule_tensor[selection_idx][i_][selection_idx] = 0
                    rule_tensor[selection_idx][project_idx][i_] = 0
                    rule_tensor[selection_idx][i_][project_idx] = 0
                    # rule_tensor[project_idx][selection_idx][i_] = 0
                    # rule_tensor[project_idx][i_][selection_idx] = 0
                    rule_tensor[project_idx][project_idx][i_] = 0
                    rule_tensor[project_idx][i_][project_idx] = 0
...
a boolean vector to tell if a given span is a gold span (i.e it corrosponds to a span_hash that is in hash_gold_levelorder[0]).
    ...
    def forward(
        ...
        orig_entities=None,
        is_gold_leaf=None,
        lengths=None, # 问句的范围和schema的范围
        offsets=None,
        relation=None, # TODO: 需要找出这个relation的计算方式
        depth=None,
        hash_gold_levelorder=None,
        hash_gold_tree=None,
        span_hash=None,
        is_gold_span=None,
        gold_value_span=None, # For debugging
        # 抽取额外候选值
        added_span_start=None,
        added_span_end=None,
        added_span_index_mask=None,
        added_span_hash=None,
        added_span_offset=None,
        is_gold_added_span=None
    ):
        # enc：问句 + 表名 + 表名.列名 进行分词、index之后的序列（ID序列）
        if 'nl2sql' in self._dataset_name:
        ...
        (
            ...
            embedded_utterance,
            utterance_mask,
            added_span_serialized,
            added_span_serialized_mask
        ) = self._encode_utt_schema(enc, offsets, relation, lengths, added_span_offset) # 将额外添加的span分离出来
        batch_size, utterance_length, _ = embedded_utterance.shape

        # 计算span准确率
        # if gold_value_span is not None:
        #     if_contain_all_span = [all(
        #         [x in torch.cat([span_hash.view(batch_size, -1), added_span_hash], dim=-1)[y] for x in gold_value_span[y]]
        #     ) for y in range(batch_size)]

        #     initial_span_acc = torch.tensor(if_contain_all_span).float().sum() / batch_size

        #     self._span_acc(initial_span_acc)
        ...
        if self.value_pred:
            ...
            ) # 通过mask确定question的范围，并取值。 span_scores[i]是第i个batch的上三角分数矩阵
            span_mask = torch.isfinite(span_scores).bool()  
            final_span_scores = span_scores.clone()
            delta = final_span_scores.shape[-1] - span_hash.shape[-1] # span_hash是问题被分词之后，在得到的序列上取所有的span计算出来的hash（如果序列由n个token构成，则有n * n个span）。is_gold_span说明对应的span是否为gold_span
            ...
            if gold_sql is not None:
                ...
                batch_idx, start_idx, end_idx = is_gold_span.nonzero().t() # nonzero: 获得目标tensor中非零元素的下标，如果是高维tensor则用[dim0, dim1, dim2]来表示其坐标，t()则是对tensor进行转置
                if self.training:
                    final_span_scores[
                        batch_idx, start_idx, end_idx
                    ] = allennlp.nn.util.max_value_of_dtype(final_span_scores.dtype) # 在final_span_scores中将gold span的分数设为最大值
                    # 训练时将final_span_scores复制出来进行改写，主要是为了下一步生成语法树时使用gold_span。而原本模型生成的span用于训练阶段的loss计算
                else:
                ...
                if self.use_bce:
                    vector_loss += ((self._bce_loss(start_logits, is_span_start) * utterance_mask).sum(-1) 
                                + (self._bce_loss(end_logits, is_span_end) * utterance_mask).sum(-1))
                    tree_sizes_vector += 2 * utterance_mask.sum(-1)
                else:
            ...
            # 将final_span_score按行优先展开成 batch_size * (n * n) 的矩阵，取topk。best_spans是topk值的下标
            best_span_scores, leaf_span_mask, best_spans = allennlp.nn.util.masked_topk(
                final_span_scores.view([batch_size, -1]),
                span_mask.view([batch_size, -1]),
                self._num_values,
            ) 
            span_start_indices = best_spans // utterance_length  # 因为view()是按行优先排列的，可以用整数除法和取余的方法得到每个batch中的每个start和end。（分数矩阵是上三角矩阵，有效值中行下标 <= 列下标，行下标为起点下标，列下标为终点下标）
            ...
            ) # 取出start_span的embedding
            end_span_rep = allennlp.nn.util.batched_index_select(
                embedded_utterance.contiguous(), span_end_indices
            ) # 取出end_span的embedding
            span_rep = (end_span_rep + start_span_rep) / 2   # span的representation是两个向量求平均
            leaf_span_rep = span_rep
            leaf_span_hash = allennlp.nn.util.batched_index_select(
                span_hash.reshape([batch_size, -1, 1]), best_spans
            ).reshape([batch_size, -1]) # 求出best_span对应的hash值

            _, added_span_num = added_span_hash.shape
            ## --- 使用抽取出的DB VALUE ---
            if added_span_num > 0:
                # 处理新引入的span
                (
                    added_span_scores, added_span_start_logits, added_span_end_logits, added_span_start_rep, added_span_end_rep
                ) = self.score_added_spans(added_span_serialized, added_span_index_mask, added_span_start, added_span_end)

                added_span_rep = (added_span_start_rep + added_span_end_rep) / 2
                final_added_span_scores = added_span_scores
                # 训练阶段修正分数
                if is_gold_added_span is not None:
                    # 计算loss
                    is_added_span_start = is_gold_added_span.float()
                    is_added_span_end = is_gold_added_span.float()
                    if self.use_bce:
                        vector_loss += ((self._bce_loss(added_span_start_logits, is_added_span_start) * added_span_index_mask).sum(-1)
                                    + (self._bce_loss(added_span_end_logits, is_added_span_end) * added_span_index_mask).sum(-1))
                        tree_sizes_vector += 2 * added_span_index_mask.sum(-1)
                    else:
                        added_span_start_probs = allennlp.nn.util.masked_log_softmax(
                            added_span_start_logits, added_span_index_mask, dim=1
                        )
                        added_span_end_probs = allennlp.nn.util.masked_log_softmax(
                            added_span_end_logits, added_span_index_mask, dim=1
                        )
                        vector_loss += (-added_span_start_probs * is_added_span_start.squeeze()).sum(-1) - (
                            added_span_end_probs * is_added_span_end.squeeze()
                        ).sum(-1)
                        # TODO: 此处的treesize是不是该*2？
                        tree_sizes_vector += 2 * (is_added_span_start.squeeze().sum(-1))
                    final_added_span_scores = added_span_scores.clone()
                    if self.training:
                        final_added_span_scores = torch.masked_fill(
                            final_added_span_scores, is_gold_added_span,
                            allennlp.nn.util.max_value_of_dtype(final_added_span_scores.dtype)
                        )
                    else:
                        final_added_span_scores = added_span_scores

                # 为添加的span评分完毕后可以修正下标了
                added_span_start = added_span_start + added_span_offset[:,1:,0] - 1
                added_span_end = added_span_end + added_span_offset[:,1:,0] - 1

                # 将added span的信息拼接进来
                span_start_indices = torch.cat([span_start_indices, added_span_start], dim=-1)
                span_end_indices = torch.cat([span_end_indices, added_span_end], dim=-1)
                best_span_scores = torch.cat([best_span_scores, final_added_span_scores], dim=-1)
                leaf_span_mask = torch.cat([leaf_span_mask, added_span_index_mask], dim=1)
                # rep和hash
                leaf_span_hash = torch.cat([leaf_span_hash, added_span_hash], dim=-1)
                leaf_span_rep = torch.cat([leaf_span_rep, added_span_rep], dim=-2)

                # 重新选择topK
                _, leaf_span_mask, best_spans = allennlp.nn.util.masked_topk(
                    best_span_scores, leaf_span_mask, self._num_values
                )

                # 重新选择: index, representation, hash
                span_start_indices = span_start_indices.gather(1, best_spans)
                span_end_indices = span_end_indices.gather(1, best_spans)
                leaf_span_rep = allennlp.nn.util.batched_index_select(leaf_span_rep, best_spans)
                leaf_span_hash = leaf_span_hash.gather(1, best_spans)
            
            # --- 使用抽取出的DB VALUE END ---

            # 计算抽取后span准确率
            if gold_value_span is not None:
                if_contain_all_span_final = [all(
                    [x in leaf_span_hash[y] for x in gold_value_span[y] if x != -1]
                ) for y in range(batch_size)]

                final_span_acc = torch.tensor(if_contain_all_span_final).float().sum() / batch_size

                self._final_span_acc(final_span_acc)
            else:
                self._final_span_acc(0)


            leaf_span_types = torch.where(
                leaf_span_mask, self._type_dict["Value"], self._type_dict["nan"]
            ).int() # 
        # 开始选择topk的DB Constant 作为叶子节点
        ...
        if gold_sql is not None:
            ...
            if not self.is_oracle:
                if self.use_bce:
                    vector_loss += (self._bce_loss(final_leaf_schema_scores.squeeze(), is_gold_leaf.float()) * schema_mask).sum(-1)
                    tree_sizes_vector += schema_mask.sum(-1)
                else:
                    ...
                    vector_loss += loss_tensor.squeeze().sum(-1) # DB CONSTANT的loss计算
        ...
        if self.value_pred:
            ...
            beam_mask = leaf_schema_mask # leaf_schema_mask计算可能有问题
            item_list.append(
                ra_postproc.ZeroItem(
                    beam_types,
                    final_leaf_indices,
        ...
        for decoding_step in range(self._decoder_timesteps):
            ...
            if self.utt_aug: # utterance augment?
                enriched_beam_rep = self._augment_with_utterance(
                    embedded_utterance,
                    utterance_mask,
                    beam_rep,
                    beam_mask, # beam_mask: 指出哪些是topK 的Zt
            ...
            l_beam_idx, r_beam_idx = vec_utils.compute_beam_idx(
                ...
                self.unary_op_count,
                device=self._device,
            ) # 所以这个l_beam_idx和r_beam_idx是什么东西？ -> 对于二元运算而言，左边的beam在beamlist中的下标和右边的beam在beamlist中的下标
            ...
            if gold_sql is not None:
                with torch.no_grad():
                    ...
                    ) # 在计算出的 frontier_hash中，是否包含了new_hash_gold_levelorder的信息。
                if self.use_bce:
                    vector_loss += (self._bce_loss(frontier_scores, is_levelorder_list) * frontier_mask).sum(-1)
                    tree_sizes_vector += frontier_mask.sum(-1)
                else:
                    avg_frontier_scores = allennlp.nn.util.masked_log_softmax(
                        frontier_scores, frontier_mask.bool(), dim=1
                    ) # 这里与loss的定义差一个负号，所以是-avg_frontier_scores
            ...
            # beam分数计算有问题？ -> mask计算有问题 -> 查看frontier_mask有没有把gold_tree给mask掉...
            ...
            l_child_idx = torch.gather(l_beam_idx, -1, beam_idx) # shape: batch_size * K
        ...
        if not self.training:
            ...
            is_final_mask = (
                self._term_tensor[beam_type_tensor].bool().to(beam_mask_tensor.device) # _term_tensor是常量
            ) # FIXME: is_final_mask的计算有问题 -> beam_type_tensor的计算有问题 -> 理论上，eval阶段中beam_type_tensor应该有14、15、16、19、20、21、23、24这几个类型，然而事实上完全没有
            ...
            ) # 这里被beam_mask_tensor将mask的score都替换成最小值了，beam_mask_tensor计算有问题
              # train阶段可以得到正常的beam_scores_tensor, 求argmax可以得到非零的下标, 但是val阶段下标总是0
              # 为什么train阶段可以? 有可能是因为train阶段的T - 1高度的子树是包含GOLD（正确答案）的，因此分数正常，而没有包含GOLD的情况下可能类型会出错。-> 实验验证：在Val的Decode阶段提供GOLD的信息，如果能正常说明应该没有额外的BUG
        # for debugging
        # if hash_gold_tree is not None and not self.training:
        #     with open('dev_err_log.json', 'w') as f:
        #         for gs, fa in zip(hash_gold_tree, beam_hash.tolist()):
        #             acc = int(gs) in fa 
        #             if acc <= 0:
        #                 cur_batch = beam_hash.tolist().index(fa)
        #                 question = self.tokenizer.decode(enc['tokens']['token_ids'][cur_batch][
        #                     lengths[cur_batch][0][0] + 1 : lengths[cur_batch][0][1] + 2
        #                 ])
        #                 current_gold = gold_sql[cur_batch]
        #                 # 查找是否有缺失的span
        #                 leaf_list = []
        #                 for hash_val in hash_gold_levelorder[cur_batch][0]:
        #                     if hash_val != -1:
        #                         if hash_val in leaf_hash[cur_batch]:
        #                             leaf_idx = leaf_hash[cur_batch].tolist().index(hash_val)
        #                             leaf_list.append(orig_entities[cur_batch][leaf_idx])
        #                         elif hash_val in span_hash[cur_batch]:
        #                             cur_start, cur_end = torch.eq(span_hash[cur_batch], hash_val).nonzero()[0]
        #                             curr_span = self.tokenizer.decode(enc['tokens']['token_ids'][cur_batch][
        #                                 cur_start + 1 : cur_end + 2
        #                             ])
        #                             curr_span.replace('##', '')
        #                             leaf_list.append(curr_span)
        #                 self.err_list.append({
        #                     'question': question,
        #                     'query': current_gold,
        #                     'leaf_list': leaf_list
        #                 })
        #         json.dump(self.err_list, f, ensure_ascii=False)
        if self.training: 
    ...
    def score_spans(self, embedded_utterance, utterance_mask): 
        logits = self._span_score_func(embedded_utterance) # 对于每一个位置的元素，计算出两个输出，一个值为该元素作为起始位置的概率，另一个值为该元素作为结束位置的概率
        ...
        ) # embedded_utterance是问题 + schema信息的混合编码，我们只需要问题的部分，因此对于其他部分利用mask将其分数替换成负的最小值
        end_logits = vec_utils.replace_masked_values_with_big_negative_number(
            end_logits, utterance_mask
        )
        span_scores = vec_utils.get_span_scores(start_logits, end_logits) # n * n 分数矩阵， 取上三角
        return span_scores, start_logits, end_logits

    def score_added_spans(self, added_span_serialized, added_span_index_mask, added_span_start, added_span_end):
        span_index = torch.stack([added_span_start, added_span_end], dim=-1)
        span_tensor = util.batched_index_select(added_span_serialized, span_index)
        # 分离两个batch的start和end
        start_tensor, end_tensor = torch.split(span_tensor, 1, dim=-2)
        start_tensor = torch.squeeze(start_tensor, dim=-2)
        end_tensor = torch.squeeze(end_tensor, dim=-2)
        # 评分, 开始端不需要end评分，结束端同理
        start_logits, _ = (self._span_score_func(start_tensor) / self.temperature).split(1, dim=-1)
        _, end_logits = (self._span_score_func(end_tensor) / self.temperature).split(1, dim=-1)
        start_logits = start_logits.squeeze(-1)
        end_logits = end_logits.squeeze(-1)
        added_span_scores = vec_utils.get_span_scores(start_logits, end_logits)
        added_span_scores = torch.diagonal(added_span_scores, offset=0, dim1=-2, dim2=-1)
        # 由序列化方法来看，下标不可能为0
        added_span_scores = added_span_scores.masked_fill(
            ~added_span_index_mask,
            allennlp.nn.util.min_value_of_dtype(added_span_scores.dtype)
        )
        return added_span_scores, start_logits, end_logits, start_tensor, end_tensor
    ...
    def _create_beam_rep(
        ...
        beam_type_rep = self.type_embedding(beam_types) # 运算符的向量表示(embedding), embedding是由torch.nn.Embedding创建的一个运算符ID -> 向量的lookup table
        ...
        beam_rep = self._pooler(beam_rep).reshape([batch_size, beam_size, emb_size]) # 将一个序列的embedding求和 -> 将3个（运算符、左、右）embedding加起来得到一个向量
    ...
    def _compute_validation_outputs(
        ...
        **kwargs,
        ...
        tree_list = []
        beam_scores_el_list = []
        evaluation_failed_sql = []
        cannot_construct_sql = []
        ...
        # eval阶段用到了sqlite，需要替换，否则EM Evaluation Failed
        if (
        ...
        if not self.training:
            ...
            for b in range(batch_size):
                ...
                top_idx = int(beam_scores_el.argmax()) # FIXME: top_idx 返回0，beam_scores完全一样
                # 尝试修正?
                if top_idx == 0:
                    items_list_num = -1
                else:
                    items_list_num = (top_idx // self._beam_size) + 2
                tree_copy = ""
                try:
                    items = kwargs["item_list"][: items_list_num]
                ...
                if hash_gold_tree is not None:
                    try:
                        ...
                        # if self._dataset_name == 'dusql':
                        #     gold_sql = fix_date(gold_sql)
                        gold_sql = kwargs["gold_sql"][b]
                        for agg in ['MAX', 'MIN', 'COUNT', 'AVG', 'SUM']:
                            if agg + '(' in gold_sql:
                                gold_sql = gold_sql.replace(agg + '(', agg + ' (')
    ...
    def _augment_with_utterance(
        ...
        if self.disentangle_cntx:
            ...
            if self.cntx_beam: # 是否在单个节点时计算Transformer(z_t)
    ...
    def emb_q(self, enc):
        ...
        # 修改pad_dim。这里的enc是完整的混合序列。如果长度超过1024会被截断
        if pad_dim > 512:
    ...
    def _encode_utt_schema(self, enc, offsets, relation, lengths, added_span_offset):
        embedded_utterance_schema = self.emb_q(enc)
        added_span_serialized = None
        added_span_serialized_mask = None
        # 从向量化的序列中取出添加的部分 确认当前操作不影响原向量，确认后续操作不影响截取的向量 -> 不会，会进行复制
        if self.value_pred:
            added_span_serialized, added_span_serialized_mask = vec_utils.batched_span_select(
                embedded_utterance_schema, added_span_offset[:,1:,:]
            )
            added_span_serialized = torch.squeeze(added_span_serialized, 1)
            added_span_serialized_mask = torch.squeeze(added_span_serialized_mask, 1)
            added_span_serialized = self._emb_to_action_dim(added_span_serialized)

        # 对向量序列进行分段。问句保持一个向量一段，表和列按表名/列名的词数进行分段([CLS]和[SEP]会被跳过) TODO: 新插入的span会在此处被忽略掉, 需要对应offsets (类似question的span进行构造)
        (
        ...
        # 将masked的部分求平均值 -> 将表名/列名部分的序列求平均值然后合并
        relation_mask = (relation >= 0).float()  # TODO: fixme
        torch.abs(relation, out=relation) # 求abs的目的是?
        embedded_utterance_schema = self._emb_to_action_dim(embedded_utterance_schema) # 1024 -> 256 线性变换
        if self._use_relation:
            enriched_utterance_schema = self._schema_encoder(
                embedded_utterance_schema, relation.long(), relation_mask
            ) # 结合relation进行Encode计算。只要确保relation计算是正确的，就可以进行复用。
        else:
            enriched_utterance_schema = embedded_utterance_schema
        # 同样按向量序列进行分段。Length标识了两段: 问题部分和schema部分，所以select后每个batch会有两段。所有batch的所有段都会被pad到最长span的长度，真实的内容长度由mask决定
        # TODO: 需要修改lengths以分离出新增的span
        ...
        return schema, schema_mask, embedded_utterance, utterance_mask, added_span_serialized, added_span_serialized_mask
    ...
    def score_frontier(self, enriched_beam_rep, beam_rep, beam_mask):
        if self.cntx_rep:
            ...
            if self.utt_aug:
                beam_rep = torch.cat([enriched_beam_rep, beam_rep], dim=-1) # 将原表示和contextualized representation拼接
                if self.lin_after_cntx:
        ...
        left = self.left_emb(beam_rep.reshape([batch_size, seq_len, 1, emb_size])) # Linear
        right = self.right_emb(beam_rep.reshape([batch_size, 1, seq_len, emb_size])) # 为什么要分left_emb和right_emb -> 为了生成不同的Zi; Zi'; Zj ; Zj' 序列，先过线性层再拼接和先拼接再经过线性层是一样的. 但这里过了一层线性层了
        binary_ops_reps = self.after_add(left + right)  # Zi; Zi' 与 Zj ; Zj'是相加而不是拼接...论文叙述有误? Linear + ReLU + Linear
        binary_ops_reps = binary_ops_reps.reshape(-1, seq_len ** 2, self.d_frontier) # 将中间的两维(K)展开为一维(K * K)
        unary_ops_reps = self._unary_frontier_embedder(beam_rep) # 这里过了：Linear + ReLU + Linear
        pre_frontier_rep = torch.cat([binary_ops_reps, unary_ops_reps], dim=1)
        pre_frontier_rep = self.pre_op_linear(pre_frontier_rep) # 为什么又要pre_op linear? Linear + ReLU

        base_frontier_scores = self.op_linear(pre_frontier_rep) # ???根据上述表示，根据每个Zi或Zi、Zj的组合得到每个运算符的一个分数? 2 * dim -> op_count 的一个线性变换层，相当于每个运算符有一个经过学习得到的评分向量
        ...
        binary_mask = torch.einsum("bi,bj->bij", beam_mask, beam_mask) # 对于一个矩阵, 
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        out = {}
        if self.training:
            out["final_beam_acc"] = self._final_beam_acc.get_metric(reset)
            out["leafs_acc"] = self._leafs_acc.get_metric(reset)
            out["cond_value_acc"] = self._final_span_acc.get_metric(reset)
        else:
            out["final_beam_acc"] = self._final_beam_acc.get_metric(reset)
