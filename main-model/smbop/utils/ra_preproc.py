    ...
    if isinstance(ast_dict, dict):
        ...
        if isinstance(ast_dict[agg_type], dict):
            ...
            if sec_agg_type in ["distinct"]:
                ...
                sec_agg_node = Node(sec_agg_type, parent=agg_type_node, n_type="Agg")  # FIXME: 这个分支没有命中!
                val1, val2 = ast_dict[agg_type][sec_agg_type]
                codegen_agg(val1).parent = sec_agg_node
                codegen_agg(val2).parent = sec_agg_node
            else:
                raise Exception
        elif isinstance(ast_dict[agg_type], list) and agg_type in ['add', 'sub', 'div', 'mul']:
            # 修复列二元运算
            val1 = ast_dict[agg_type][0]
            val2 = ast_dict[agg_type][1]
