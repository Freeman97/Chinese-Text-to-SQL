...
class LxmertCrossAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        attention_output = self.output(output[0], input_tensor) # Linear、残差链接、LayerNorm
        outputs = (
...
class LxmertAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        ...
        x = x.view(*new_x_shape) # 拆分成8个head 8 * 32
        return x.permute(0, 2, 1, 3) # batch_size * num_head * sequence_len * embedding_dim

    def forward(
        self, hidden_states, context, attention_mask=None, output_attentions=False
    ):
        mixed_query_layer = self.query(hidden_states) # 矩阵Q
        mixed_key_layer = self.key(context) # 矩阵K
        mixed_value_layer = self.value(context) # 矩阵V
        ...
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2)) # Q * K
