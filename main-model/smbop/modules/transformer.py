...
def relative_attention_logits(query, key, relation):
    ...
    qk_matmul = torch.matmul(query, key.transpose(-2, -1)) # qk_matmul[i, j] i 对 j 的权重值。

    # q_t is [batch, num queries, heads, depth]
    q_t = query.permute(0, 2, 1, 3) # [batch, heads, num queries, dim (action dim / H)] -> [batch, num queries, heads, dim]
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, relation, mask):
        ...
        # 用完记得删...
        # relation_k = relation_k * 0
        # relation_v = relation_v * 0
