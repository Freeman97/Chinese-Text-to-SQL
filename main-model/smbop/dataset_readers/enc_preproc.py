...
class EncPreproc:
    ...
    def __init__(
        ...
        self._tables_file = tables_file # tables.json 
        self._dataset_path = dataset_path # sqlite files  
        ...
        # 不使用SQLITE, 而是使用json文件
        with open(dataset_path + 'db_content.json', 'r') as f:
    ...
    def compute_relations(
    ...
    ):  # 去掉模式连接：sc_link和cv_link都置空
        sc_link = desc.get("sc_link", {"q_col_match": {}, "q_tab_match": {}}) # sc_link: 问句token index -> 表ID， 问句token index -> 列ID，匹配依据是表名和列名
        cv_link = desc.get("cv_link", {"num_date_match": {}, "cell_match": {}}) # cv_link: 问句token index -> 列ID，匹配依据是列值
        # 需要理清relations的计算过程
        # Catalogue which things are where
        loc_types = {} # loc_types: 当前位置的类型。TODO: 序列化方式和最终的序列化方式不一致
        for i in range(q_enc_length):
            loc_types[i] = ("question",)
        # 标注当前位置的类型: 问题文本: ("question",)  列：("column", 列ID)  表: ("table, 表ID")
        c_base = q_enc_length # 这个cstart和cend的循环真的很复杂..但实际上是等于插入[列数]个列类型
        ...
        # 完整的Relation还包括Auxiliary Relations for Schema Encoding: 问题tokens之间的联系, 同一表名/列名的联系, 任意表/列之间的联系
        for i, j in itertools.product(range(enc_length), repeat=2):
            ...
            if i_type[0] == "question":
                if j_type[0] == "question":
                    set_relation(("qq_dist", clamp(j - i, self.qq_max_dist))) # qq_dist: 问题token间的偏置
    ...
    def preprocess_item(self, item, validation_info):
        ...
        question = [x.replace("▁", "") for x in question] # 分词分出来的特殊字符 TODO: BERT系列特殊字符##
        question_for_copying = [x.replace("▁", "") for x in question_for_copying]
        ...
        # print(sc_link)
        # 千言数据集中Cspider db_content.json 有错
        try:
    ...
    def compute_cell_value_linking(self, tokens, schema):
        ...
        def db_word_match(word, column, table, db_id):
            # 表table的column列是否包含word
            # 将SQLITE查询替换成JSON数据查找
            # return False #fixme
            ...
            for db in self.db_content:
                if db_id == db['db_id']:
                    schema = db
                    break
            
            if schema is None:
                schema = self.db_content[0]

            table_data = schema['tables'][table]
            # 先确定列
            list_index = table_data['header'].index(column)
            # 查找
            ret = []
            for row in table_data['cell']:
                value = row[list_index]
            ...
            for col_id, column in enumerate(schema.columns):
                ...
                if num_flag:
                    if column.type in ["number", "time"]:  # TODO fine-grained date
                        ...
                        # num_date_match[f"{q_id},{col_id}"] = column.type.lower()
                else:
                    if self.filter_longdb(schema.db_id):
                        ret = db_word_match(
                            word,
                            column.unsplit_name,
                            column.table.unsplit_name,
                            schema.db_id,
                        )
                        if ret:
                            # print(word, ret)
