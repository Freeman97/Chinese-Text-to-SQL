...
try:
    from smbop.eval_final.process_sql import (
        ...
        Schema,
        get_sql,
        get_sql_using_sqlparse,
        get_sql_using_dusql_tokenizer
    )
except:
    from process_sql import tokenize, get_schema, get_tables_with_alias, Schema, get_sql

from smbop.eval_final.dusql_evaluation import evaluate_complex_single, evaluate_NL2SQL
...
table_dict = None
# p_str: 生成的SQL语句
def evaluate_single_using_json(g_str, p_str, db_id, db_dir, table_file, value_match=True):
    # 对于不使用SQLite的数据集需要使用其他的eval方法
    # db_dir: db_contents的路径
    global table_dict
    if 'dusql' in table_file or 'debug' in table_file or 'cspider' in table_file:
        return evaluate_complex_single(p_str, g_str, table_file, db_id, value_match=value_match)
    elif 'nl2sql' in table_file:
        # 文件太大，提前读入内存
        ...
        return evaluate_NL2SQL(p_str, g_str, table_dict, db_id)

    kmaps = build_foreign_key_map_from_json(table_file)
    evaluator = Evaluator()
    schema = Schema(get_schema_from_json(db_dir, db_id)) # Eval的时候此处得到的
    try:
        if 'dusql' in table_file or 'debug' in table_file:
            g_sql = get_sql_using_dusql_tokenizer(schema, g_str)
        else:
            g_sql = get_sql_using_sqlparse(schema, g_str)
    ...
    try:
        if 'dusql' in table_file or 'debug' in table_file:
            p_sql = get_sql_using_dusql_tokenizer(schema, p_str, silent=True)
        else:
            p_sql = get_sql_using_sqlparse(schema, p_str)
    except Exception as e:
        # If p_sql is not valid, then we will use an empty sql to evaluate with the correct sql
        # 不要吞掉异常
        # logging.exception(e)
...
def evaluate_single(g_str, p_str, db_id, db_dir, table_file):
    ...
    schema = Schema(get_schema(db))  # get_schema方法返回的是一个Dict[str, List[Str]]，即表名 -> [列名] （都英文）
    g_sql = get_sql(schema, g_str)
