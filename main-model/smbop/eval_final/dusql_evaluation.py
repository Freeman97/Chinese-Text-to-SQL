...
def parse_value(toks, start_idx, tables_with_alias, schema, default_tables=None):
    ...
    elif (toks[idx].startswith('"') and toks[idx].endswith('"')) or toks[idx] == 'value':  # token is a string value
        val = toks[idx]
        idx += 1
    else:
        try:
...
def evaluate_complex_single(pred_str, gold_str, table_file, db_id, value_match=True):
    ...
    return evaluator.eval_exact_match(pred_sql, gold_sql, value_match=value_match)
