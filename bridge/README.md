# BRIDGE model
**针对中文数据集DuSQL, TableQA及CSpider进行了一定的调整和扩展**
* 参考了BRIDGE原文的[实现代码](https://github.com/salesforce/TabularSemanticParsing)
* 对中文数据及进行实验的运行配置请参考`launch.json`（vscode配置文件）
* 对于DuSQL及TableQA数据集，准备数据集后请先运行`remove_corrupted_case.py`
* 对于TableQA数据集，进行数据预处理前需要运行`process_sql.py`及`tableqa_preprocess.py`
* 推理结果需要运行`output_postprocess.py`对结果进行后处理才能进行准确率评估
* 对原文的fuzzy string algorithm进行了调整：中文自然语言没有空格作为词或短语的"边界"。匹配的两个阈值没有进行修改。
